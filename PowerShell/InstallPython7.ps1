...
# Enter list of computers to upgrade python into d:\powershell\targets.txt one per line no white space an the end of the line.
$colComputer = Get-Content d:\PowerShell\ITK_win.txt

# Network Mapping
Write-Host "Enter Password for nlmlhcaa on mini9"
net use \\mini9\Data /u:nlmlhcaa
...
Foreach ($strComputer in $colComputer){
	...
	#Write-Verbose Invoke-Command -ComputerName $strComputer -FilePath $LocalPS -ArgumentList "-Verbose"
	#Invoke-Command -ComputerName $strComputer -FilePath $LocalPS -ArgumentList "-Verbose"
	
	$delFile = $rmtFile + $pipUpgrade
	If (Test-Path $delFile){
		#Clean up, remove install file
		Write-host Remove-Item $delFile -force
		#Remove-Item $delFile -force
	}
}
